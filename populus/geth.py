...
def get_latest_known_instances(contracts, data_dir):
    """ Given the current compiled contract info, this function
        will search for the current known instances in the passed
        active directory that were created with code that matches
        the current code.
        The idea here is that often during development, the user will
        deploy a contract, find/fix bugs, recompile, and then redeploy.
        This method will only get known contracts that match the latest
        code.
        @param contracts dict of the contract objects (generated by
            the "package_contracts" method
        @param data_dir data directory for the active project chain.
        @return dict whose keys are the name of the contracts and the
            values of each key are a list of tuples in the form
            (ts, addr) where the ts is a datetime timestamp, and addr
            is the address of a particular contract instance.
    """
    known_cts = get_known_contracts(data_dir)
    ret = {}
    for name, ct_type in contracts:
        ret[name] = []
        if name in known_cts.keys():
            ...
            for data in addr_list:
                if curr_code_hash == data["codehash"]:
                    ...
                    ret[name].append((ts, data["address"]))
    return(ret)
