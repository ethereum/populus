...
def test_getting_source_file_for_package_contract(project):
    source_file_path = get_contract_source_file_path(project.compiled_contract_data['owned'])
    expected_path = os.path.join(
        project.installed_dependency_locations['owned'],
        'contracts',
        'owned.sol'
    )
    assert is_same_path(source_file_path, expected_path)
