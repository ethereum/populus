...
PROJECT_CONFIG_SCHEMA_FILENAMES = {
    V7: "config.v7.schema.project.json",
}
...
def get_project_config_schema_path(version=LATEST_VERSION):
    try:
        return os.path.join(ASSETS_DIR, PROJECT_CONFIG_SCHEMA_FILENAMES[version])
    except KeyError:
        raise KeyError(
            "`version` must be one of {0}".format(
                sorted(tuple(CONFIG_SCHEMA_FILENAMES.keys()))
...
def load_config_schema(version=LATEST_VERSION, context=USER_CONFIG_CONTEXT):
    if context == USER_CONFIG_CONTEXT:
        config_schema_path = get_config_schema_path(version)
    elif context == PROJECT_CONFIG_CONTEXT:
        config_schema_path = get_project_config_schema_path(version)
    config_schema = anyconfig.load(config_schema_path)
    return config_schema


def load_project_config_schema(version=LATEST_VERSION):

    return load_config_schema(version, context=PROJECT_CONFIG_CONTEXT)


@to_tuple
def get_validation_errors(config, version=None, schema=None):
    if schema is None:
...
def validate_config(config, version=None, schema=None):
    errors = get_validation_errors(config, version, schema)
    if errors:
