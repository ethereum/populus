...
def test_testrpc_chain_has_registrar(project):
    with project.get_chain('testrpc') as chain:
        assert chain.has_registrar is True


def test_tester_chain_has_registrar(project):
    with project.get_chain('tester') as chain:
        assert chain.has_registrar is True


def test_temp_chain_has_registrar(project):
    with project.get_chain('temp') as chain:
        assert chain.has_registrar is True


def test_geth_chain_has_registrar(project):
